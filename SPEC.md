...
|       +-- ranker_user.md
|       +-- context/
|           +-- past_themes.md      # Auto-populated by update_themes node (Tier 2 memory)
+-- outputs/
|   +-- global_history/
|       +-- idea_history.jsonl      # {name, text} per idea across all runs
|       +-- idea_vectors.npy        # Numpy embedding array for cosine similarity
|   +-- runs/<timestamp>/
|       +-- raw_ideas.jsonl
...
|---------------|-----------------|---------|
| grok-4.1-fast | x-ai/grok-4.1-fast | Ideator (A), Angel's Rescue (C sub-routine) |
| deepseek-v3.2 | deepseek/deepseek-v3.2 | Recombiner (B), DSR Designer (E) |
| gpt-oss-120b | openai/gpt-oss-120b | Gatekeeper (C), Principles Judge (D), Pre-Ranker (D.5), Ranker (F) |
| glm-4.7-flash | z-ai/glm-4.7-flash | Schema Repair (utility fallback) |

### 5.3 Temperature Configuration

| Agent | Temperature | Rationale |
|-------|-------------|-----------|
| Ideator | 0.85 | Encourage creativity |
| Recombiner | 0.60 | Balance creativity and coherence |
| Gatekeeper | 0.20 | Consistent evaluation |
| Principles Judge | 0.18 | Consistent scoring |
| Pre-Ranker | 0.20 | Consistent scoring |
| DSR Designer | 0.25 | Structured reasoning |
| Ranker | 0.15 | Deterministic ranking |

### 5.4 Schema Repair Mechanism
On any schema validation failure from a primary agent:
1. Do NOT retry with the primary agent
2. Immediately route the malformed output + target Pydantic schema to the **glm-4.7-flash** repair agent
...
Node F: Ranker (aggregate, rank, 7-day plan)
              |
Node G: Update Themes (refresh overused-theme list for Tier 2 memory, 10+ ideas threshold)
              |
             END
```
...
**Two-Tier Memory System (v3.1):**

The ideator integrates a two-tier memory system to prevent generating duplicate or thematically stale ideas across runs.

- **Tier 1 (Hard Filter):** Each generated idea is embedded and compared via cosine similarity against a global history of all previously generated ideas. Ideas exceeding the `similarity_threshold` (default 0.85) are silently dropped. Intra-batch detection is included -- each accepted idea within a batch is added to the in-memory history so later ideas in the same batch are checked against earlier ones. When Tier 1 removes ideas below the target count, up to 2 backfill rounds cycle through segments to generate replacement ideas. Each backfill batch also passes through the novelty filter.
- **Tier 2 (Soft Prompt):** A `past_themes.md` file (auto-generated by the `update_themes` node) is injected into the ideator system prompt via the `{past_themes}` template variable. This file contains a bulleted list of overused themes distilled from global history, nudging the LLM to explore fresh territory without hard-blocking any specific direction.
...
### Node G: UPDATE THEMES (Post-Pipeline Maintenance)

**Purpose:** Refresh the overused-themes list for Tier 2 soft memory

| Parameter | Value |
|-----------|-------|
| Input | Global idea history (all runs) |
| Output | Updated `src/prompts/context/past_themes.md` |
| Model | grok-4.1-fast |
| Temperature | 0.30 |
| Trigger | Only fires when 10+ ideas exist in global history |

**Logic:**

1. Load all idea names from `idea_history.jsonl`
2. Ask the LLM to summarize recurring themes into a bulleted list of overused directions
3. Write the result to `past_themes.md` (overwrite)
4. This file is injected into the ideator system prompt on the next run via the `{past_themes}` template variable

**Failure handling:** Non-fatal. Wrapped in try/except so a failed LLM call does not crash the pipeline. If it fails, the previous `past_themes.md` content is preserved.
...
# Two-Tier Memory System (novelty filtering + theme nudging)
memory:
...
2. Immediately route to glm-4.7-flash repair agent (no retry with primary)
...
Decisions made during design interviews (v2.0) and subsequent enhancements (v3.0, v3.1):
...
| 10 | Schema repair | Immediate fallback to glm-4.7-flash utility agent |
| 11 | Reservation test | Structured WTP assessment |
| 12 | Hybrid count | Unconstrained with minimum 5 |
| 13 | Ranker model | GPT-OSS 120B (same as Judge) |
...
| 33 | Angel's Rescue | Sub-routine in gatekeeper before kill confirmation (grok-4.1-fast) |
...
| 38 | Tier 1 Memory | Embedding cosine similarity (threshold 0.85) against global history. Intra-batch dedup included. |
| 39 | Tier 2 Memory | past_themes.md injected into ideator system prompt. Auto-generated after 10+ ideas in history. |
| 40 | Global History | Dual-file: idea_history.jsonl (human-readable) + idea_vectors.npy (numpy embeddings). Sanity check resets on mismatch. |
| 41 | Backfill Logic | Up to 2 backfill rounds when Tier 1 drops ideas below target count. |
| 42 | Update Themes Node | Runs after ranker, before END. Non-fatal (try/except wrapped). |
| 43 | Pipeline Flow v3.1 | ranker -> update_themes -> END (was ranker -> END) |
| 44 | LLM Model Swap | Replaced all models: grok-4.1-fast (creative), deepseek-v3.2 (synthesis/reasoning), gpt-oss-120b (judging/scoring), glm-4.7-flash (utility). Config-only change. |

---

*End of Specification Document v3.1*
